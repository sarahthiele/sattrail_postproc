...
outputfiles = [os.path.join(outpath, subfile.split('/')[-1].split('-sub.fits')[0] + '-sattrail.hdf') for subfile in subfiles]
...
for i, subfile in enumerate(subfiles[:2]):
    ...
    df0, traillist, mask_master, cpoints, rpoints = pp.postproc(subfiles[i], detfiles[i], outputfiles[i],
                                                                plotfiles[i], save=SAVE, plot=PLOT, skeleton=SKELETON,
                                                                progressive=PROG, gpu=GPU, filter_radius=FILTR,
